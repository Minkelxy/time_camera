...
void setup() {
  ...
  Serial.println();
  Serial.println("=== ESP32-CAM 户外定时拍照装置 v3.0 ===");
...
bool captureAndSavePhoto() {
  ...
  String filename = "/photos/photo_" + timestamp + ".jpg";
...
<body>
  ...
  if (sdCardAvailable) {
    html += "<div class='section'><div class='photos'>";
    File folder = SD_MMC.open("/photos");
    if (folder && folder.isDirectory()) {
      File file = folder.openNextFile();
      while (file) {
        if (!file.isDirectory() && String(file.name()).endsWith(".jpg")) {
          html += "<div class='photo'>";
          html += "<img src='/download?file=" + String(file.name()) + "'>";
          html += "<div class='info'>" + String(file.name()) + " (" + String(file.size()/1024) + " KB)</div>";
          html += "<div class='actions'><a href='/download?file=" + String(file.name()) + "'>下载</a></div>";
          html += "</div>";
        }
